...
export default function TokenLightweightChart({
  ...
  const buildLiveCandles = useCallback((source: CandleWithVolume[]): CandleWithVolume[] => {
    const cp = typeof currentPrice === 'number' && currentPrice > 0 ? currentPrice : null;
    const nowBucket = getBucketTime(nowSec, intervalSec);

    const createdAtSec = (() => {
      if (typeof createdAt === 'number' && createdAt > 0) {
        const raw = createdAt > 1_000_000_000_000 ? Math.floor(createdAt / 1000) : Math.floor(createdAt);
    ...
    const startBucketRaw = createdAtSec !== null ? getBucketTime(createdAtSec, intervalSec) : nowBucket;
    ...
    const firstSourceOpen = sourceBuckets.length > 0 ? Number(bucketMap.get(sourceBuckets[0])?.open) : null;
    ...
    const shouldApplyCurrentPrice = cp !== null && (source.length === 0 || latestSourceBucket === nowBucket);
    ...
    return filled;
  }, [createdAt, currentPrice, intervalSec, nowSec]);
  ...
  }, [theme, themeColors.background, themeColors.borderColor, themeColors.crosshairColor, themeColors.crosshairLabelBg, themeColors.gridColor, themeColors.textColor, interval]);
