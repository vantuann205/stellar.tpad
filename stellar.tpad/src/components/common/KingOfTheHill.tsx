...
const fmtMC = (mc: number): string => {
  const val = mc > 0 ? mc : 0.0001 * 1_000_000_000;
  return formatMarketCap(val);
};

const fmtVol = (vol: number | undefined): string => {
  if (!vol || vol <= 0) return '$0.00';
  return '$' + formatVolume(vol);
};

const fmtPrice = (priceHistory: { time: string; price: number }[] | undefined): string => {
  if (priceHistory && priceHistory.length > 0) {
    const p = priceHistory[priceHistory.length - 1].price;
