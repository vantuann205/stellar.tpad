...
impl BondingCurveContract {
    pub fn __constructor(env: Env, admin: Address, treasury: Address, xlm_address: Address) {
        set_config(&env, &admin, &treasury, &xlm_address);
    }

    pub fn initialize(env: Env, admin: Address, treasury: Address, xlm_address: Address) {
        set_config(&env, &admin, &treasury, &xlm_address);
    }
    ...
    pub fn register_token(env: Env, token_address: Address, token_admin: Address) {
        ...
        env.storage()
            .persistent()
            .extend_ttl(&key, 120_000, 250_000);
        env.events()
            .publish((symbol_short!("register"), token_address), ());
    }
...
fn set_config(env: &Env, admin: &Address, treasury: &Address, xlm_address: &Address) {
